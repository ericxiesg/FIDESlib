...
**我没能只靠读源码把这 3 格算平。** 如果是真的，代价是 **3 格 × 12 层**，值得单独查一次：
把两边的 `evaluate_polynomial` 在同一组系数上逐步打 level，对齐每一步。
也可能是笔记里的 "14 → 3" 本身是约数（它标了"约"）。
...
**要查的一处：GELU 那 3 格**（3.4）。如果是真的，12 层就是 36 格，
比上面那个优化更值钱。
