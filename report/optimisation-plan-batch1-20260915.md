...
| ⑤ | light plaintext 缓存容量 | 成立但**应当拒绝** | **[实测] 见下 3.1**。Part17 建议提高容量，方向是反的：提高只花显存、几乎不换命中率。库默认是 64，但 **bench 用的是 8**（`bench.py:834`），已经是低内存那一端。**不改** | **放弃** |
...
### 3.1 ⑤ 的实测：明文缓存提高容量没有收益 [实测]

一层 21195 次明文乘法，涉及 674 个不同明文内容、**959 个不同 (内容, level) 组合**——
后者才是这个缓存的键（`api/CryptoContext.cpp:2293` `GetExpandedLightPlaintext`，
键是 `(lp->uid, level)`）。拿真实访问序列跑 LRU：

| 容量 | 未命中 | 命中率 | 缓存峰值 |
|---:|---:|---:|---:|
| 0 | 21195 | 0.0% | 0 M |
| **8（bench 现用）** | 1477 | **93.0%** | **124 M** |
| 16 | 993 | 95.3% | 246 M |
| 32 | 967 | 95.4% | 486 M |
| 64（库默认） | 967 | 95.4% | 966 M |
| 959（全装下） | 959 | **95.5%** | 7404 M |

条目大小按 `(level+1) * N * 8`：缓存里放的是**展开后的设备明文**，不是轻明文。

两条结论：

1. **提高容量买不到东西。** 8 → 32 多花 362 MiB，换 2.4 个百分点的命中率
   （510 次展开／21195 次乘法）。一次展开是 512 KiB 上传加设备端展开，约 0.2 ms，
   510 次合计 ~0.1 s——对一层几十秒毫无意义。再往上，全装下要 7.4 GiB 换 0.1 个百分点。
2. **访问是突发式的**，所以小缓存就能吃掉绝大部分：容量 4 和 8 的未命中数完全相同（1477），
   32 和 64 也完全相同（967）。这解释了为什么曲线在很早就压平。

所以 ⑤ 不做，`--light-plaintext-cache 8` 保持不变。实测数已写进该选项的 help，
免得以后重新翻这笔账。
...
**⑤ 和 ⑧ 放弃**（⑤ 已实测否掉，见 3.1；⑧ 在 batch_size=1 下不存在）。
⑦ 只补真引擎路径上的断言。
